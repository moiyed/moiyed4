...
def start(bot, update):
    ...
    bot.send_message(chat_id=chat_id, text=f"تم الاشتراك! سأرسل لك تحليلًا تلقائيًا. Chat ID: {chat_id}")
    updater.job_queue.run_repeating(check_profitable_trade, interval=300, context=chat_id)

def check_profitable_trade(bot, job):
    chat_id = job.context
    if chat_id in subscribed_users:
        try:
            response = td.time_series(symbol="EUR/USD", interval="1h", outputsize=5).as_json()
            if response and isinstance(response[0], dict):  # ده السطر اللي فيه المشكلة
